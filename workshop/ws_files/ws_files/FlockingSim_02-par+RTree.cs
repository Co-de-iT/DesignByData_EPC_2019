...
public class Script_Instance : GH_ScriptInstance
{
    ...
    private void RunScript(bool Reset, bool Play, bool is3D, int Count, double TimeStep, double NeighborRad, double Alignment, double Cohesion, double Separation, double SepDist, List<Circle> Repellers, bool UseParallel, bool UseRTree, ref object Positions, ref object Velocities)
    {
        ...
        // part 02 - parallelization and RTree implementation

        //List<GH_Point> positions = new List<GH_Point>();
        //List<GH_Vector> velocities = new List<GH_Vector>();

        // in case of a variable number of agents initialize these
        // after the call to flockSystem.Update() with flockSystem.Agents.Count
        positions = new GH_Point[Count];
        velocities = new GH_Vector[Count];
        ...
        else
        {
            ...
            {
                positions[i] = new GH_Point(flockSystem.Agents[i].Position);
                velocities[i] = new GH_Vector(flockSystem.Agents[i].Velocity);
            });
    ...
    // transformed the output geometry variables into arrays
    // for faster output via parallelization
    GH_Point[] positions;
    GH_Vector[] velocities;
    ...
    public class FlockSystem
    {
        ...
        /* this function is defined to package the update actions in a void function
         * so to kmake use of the void delegate in the parallel computation case
         */
        ...
        {
            RTree rTree = new RTree(); // we build a new RTree every update

            // note: RTree cannot be parallelized
            ...
            foreach (FlockAgent agent in Agents)
            {
                ...
                rTree.Search(new Sphere(agent.Position, NeighborhoodRadius), rTreeCallback);
                agent.ComputeDesiredVelocity(neighbours);
            }

            UpdateAgentsPositions();
            //var part = Partitioner.Create(0, Agents.Count);
            //Parallel.ForEach(part, (range, loopstate) =>
            //{
            //    for (int i = range.Item1; i < range.Item2; i++)
            //        Agents[i].UpdateVelocityAndPosition();
            //});
        ...
        public void Update()
        {
            if (!UseParallel)
                ...
                /* Parallelizing - phase 1
                 * implementing a void function to be used by the Parallel.ForEach delegate
                 * remember to update the "using" statements the first time
                 * the Parallel static class is used:
                 * using System.Threading.Tasks;
                 * (also in the C# component in GH)
                 */
                // Parallel.ForEach(Agents, ComputeAgentDesiredVelocity);

                /* Parallelizing - phase 2
                 * after parallelizing by using a delegate function (see line above)
                 * I can use anonymous functions to write the function in place
                 * like here below */
    ...
    public class FlockAgent
    {
        ...
        {
            /* this is a more elegant way of steering:
             classical method would have a steering vector newVel = desired-vel
             added to the velocity itself. In the way used here, gradual change
             of direction is implemented algebrically
             */
            Velocity = 0.97 * Velocity + 0.03 * desiredVelocity;

            // contain velocity vector between min and max (make external parameters?)
            if (Velocity.Length > 8.0) Velocity *= 8.0 / Velocity.Length;
            else if (Velocity.Length < 4.0) Velocity *= 4.0 / Velocity.Length;

            Position += Velocity * FlockSystem.TimeStep; // TimeStep can also be considered as the maxForce
        ...
        public void ComputeDesiredVelocity(List<FlockAgent> neighbours)
        {
            ...
            // implementation: transform these into external parameters
            double boundingBoxSize = 30.0;
            double containmentStrength = 20.0; // added this variable to enhance containment
